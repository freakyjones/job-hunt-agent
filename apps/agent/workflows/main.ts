...
async function logFatalErrorToSupabase(error: Error, type: string) {
  try {
    logger.error(
      `[${type}] Intercepted fatal crash. Attempting to notify Supabase...`,
      error.stack
    );
    ...
    logger.error(
      'Failed to log fatal error to Supabase (Network down?). Proceeding to crash.',
      supabaseErr.message
    );
  } finally {
    process.exit(1);
...
async function runScrape(db: DBStateManager) {
  ...
  try {
    const rssJobs = await scrapeRssFeed(
      'https://weworkremotely.com/categories/remote-programming-jobs.rss'
    );
    const atsRssJobs = rssJobs.filter(
      (j) => j.url.includes('lever.co') || j.url.includes('greenhouse.io')
    );
    atsRssJobs.forEach((job) =>
      atsJobs.push({
        title: job.title || 'Remote Developer (WWR)',
        company: job.company || 'WWR Startup',
        description: '',
        url: job.url,
        atsType: job.url.includes('lever') ? 'lever' : 'greenhouse',
      })
    );
  } catch (e: any) {
  ...
  console.log(
    `Found ${boardJobs.length} board jobs, ${atsJobs.length} ATS jobs. Selected 50/50 mix (${shuffledBoards.length} + ${shuffledAts.length}).`
  );
  ...
  if (pendingJobs.length > 0) {
    try {
      ...
      console.log(`Pushed ${pendingJobs.length} jobs to the database for deduplication.`);
    } catch (e: unknown) {
      ...
      try {
        ...
        if (e instanceof Error) {
          ...
          if (e.message.includes('429')) {
            ...
            console.log(
              `Hit rate limit. Exponential backoff: Waiting ${backoff / 1000} seconds...`
            );
...
async function runApply(db: DBStateManager) {
  ...
  acceptedJobs = acceptedJobs.filter(
    (j) => j.url?.includes('lever.co') || j.url?.includes('greenhouse.io')
  );
    ...
    for (const job of acceptedJobs) {
      ...
      try {
        ...
        if (masterResume && job.description) {
          try {
            customResumePath = await tailor.tailorResume(
              masterResume,
              job.description,
              job.company
            );
            ...
            console.error(
              `Tailoring failed for ${job.company}, falling back to master resume:`,
              e.message
            );
          }
        }
        ...
        if (success) {
          ...
          console.error(
            `Could not auto-apply to ${job.company}. Ensure ATS is supported and job is open.`
          );
          await db.updateJobStatus(job.id, JobStatus.ERROR);
        }
      } catch (err: any) {
