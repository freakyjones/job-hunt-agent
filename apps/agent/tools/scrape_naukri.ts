 ...
 */
export async function scrapeNaukri(keyword: string = "react developer", location: string = ""): Promise<JobDetails[]> {
    ...
    try {
        ...
        for (let i = 0; i < Math.min(jobCards.length, 5); i++) { // Limit to 5 for now
            ...
            try {
                ...
                console.error("Error parsing a Naukri job card:", cardError);
            }
        }
