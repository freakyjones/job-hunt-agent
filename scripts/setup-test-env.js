...
try {
  ...
  // 3. Match keys using regex
  const anonKeyMatch = cleanStdout.match(/anon key:\s*(\S+)/);
  const serviceRoleKeyMatch = cleanStdout.match(/service_role key:\s*(\S+)/);
  const apiUrlMatch = cleanStdout.match(/API URL:\s*(\S+)/);
